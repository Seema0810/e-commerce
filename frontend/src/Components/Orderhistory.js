...
// calling the get request to show the order history of the customer
    const getOrderHistory = async () => {
        const res = await axios.get(`${API_BASE_URL}/api/order/custId`, { headers });
  ...
  return (
    <> 
    <h1 className='text-center' style={{marginTop:"5rem"}}>Orders by me </h1>
      ...
      <tbody>
        {orders.map((order)=>{
            return(
                ...
                <td className='text-center'>wpmen's shirt</td>
